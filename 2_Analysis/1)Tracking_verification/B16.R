...
rm(list=ls())
## Open file ----
setwd("/Users/martin/Documents/GitHub/Brain_Like_A_Sieve/1_Data/1_Raw")
filename="B16"
...
raw4<-read.table(file=paste(filename,"_D_tracked.csv",sep=""), sep=",",header=T)
max(raw3$frame)
r2<-raw2 %>% 
  mutate(frame=frame+max(raw1$frame))
r3<-raw3 %>% 
  mutate(frame=frame+max(r2$frame))
r4<-raw4 %>% 
  mutate(frame=frame+max(r3$frame))
raw<-bind_rows(raw1,r2,r3,r4)
...
write.table(IR2, file=paste(filename, "_B.csv", sep=""))
