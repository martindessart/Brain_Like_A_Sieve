...
#Because video was divided in two parts, manually adjust TRIAL 1 to 3:

Sti_range<-Sti_range %>% 
  filter(Trial!=1) %>% 
  filter(Trial!=2) %>% 
  filter(Trial!=3) %>% 
  add_row(Trial=1,frameD=1203,frameF=frameD+74) %>% 
  add_row(Trial=2,frameD=8707,frameF=frameD+74) %>% 
  add_row(Trial=3,frameD=16215,frameF=frameD+74) %>% 
  arrange(Trial)

Display_response(Sti_range,1) # input = Df + trial number to plot
Display_response(Sti_range,2) # input = Df + trial number to plot
Display_response(Sti_range,3) # input = Df + trial number to plot
...
adjust <- 28 #frames to adjust precisely the location of the 1st trial
...
Display_response_TEST <-function(ranges,nb_Sti) {
  ...
  ggplot(sti)+ #Display data
    ...
    theme_light()+
    geom_rect(data=ranges,aes(xmin=deb, xmax=fin, 
                              ymin=(-400), ymax=(0)), fill='#0377d709',alpha=0.1)
}
